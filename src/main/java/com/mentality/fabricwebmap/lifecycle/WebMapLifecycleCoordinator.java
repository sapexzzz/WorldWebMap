...
public final class WebMapLifecycleCoordinator {
    ...
    public WebMapLifecycleCoordinator(Settings settings, RenderService render, WebFactory webFactory) {
        this.settings = settings; this.render = render; this.webFactory = webFactory;
    }
    ...
    public void startOrStop() throws Exception {
        ...
        render.setServer(server);
        if (!render.isRunning()) render.start();
        if (web == null || !web.isRunning()) startWebService();
    }
    public void reload(String oldBind, int oldPort) throws Exception {
        startOrStop();
        if (settings.enabled() && web != null && web.isRunning()
                && (!settings.bindAddress().equals(oldBind) || settings.port() != oldPort)) {
            web.stop();
            web = null;
            startWebService();
        }
    ...
    public void stop() {
        if (render.isRunning()) render.stop();
        if (web != null) { web.stop(); web = null; }
    }
    private void startWebService() throws Exception {
        WebService replacement = webFactory.create();
        replacement.setServer(server); // Install server before accepting HTTP requests.
        replacement.start();
        web = replacement;
    }
}
