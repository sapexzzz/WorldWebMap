...
public final class RenderJob {
     ...
     * Use for command-triggered renders so the whole tile is fully rendered.
     * For auto-renders (ChunkLoadListener) keep false — the chunk is already
     * in memory when the event fires, and loading neighbours would waste time.
     */
    ...
    /** Backwards-compat: loadChunks=false (auto-render default). */
    public RenderJob(ResourceKey<Level> dimension, int tileX, int tileZ, int zoom,
                     Priority priority, boolean force) {
        this(dimension, tileX, tileZ, zoom, priority, force, false);
    }

    /** Convenience constructor with normal priority and no force. */
