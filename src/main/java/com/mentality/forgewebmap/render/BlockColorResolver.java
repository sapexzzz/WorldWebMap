 ...
 *
 * Only pure block-type lookup here (no biome tinting in MVP).
 * Biome colors can be added later as a post-process step.
 *
 * NOTE: Block references (Blocks.GRASS_BLOCK etc.) are Forge/Minecraft
 * registry objects — safe to use server-side.
 */
public final class BlockColorResolver {

    private static final int FALLBACK = 0x555555;

    /** Packed RGB (no alpha) per Block. Built once at class load. */
    ...
    static {
        ...
        // ── Misc ─────────────────────────────────────────────────────────────
        put(Blocks.AIR,              0x000000); // transparent / void — handled specially
        put(Blocks.CAVE_AIR,         0x000000);
        put(Blocks.VOID_AIR,         0x000000);
        put(Blocks.FARMLAND,         0x7A5030);
        put(Blocks.GRASS,            0x7AC850); // tall grass top
    ...
    /**
     * Resolve the top-down color for a given BlockState.
     *
     * @param state the block state at the surface
     * @return packed RGB (no alpha)
     */
