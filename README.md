...
Server-side web map for Minecraft **1.20.1**. This branch is the Fabric loader line only; it is not a `fabric/` directory in a combined repository. Forge lives on its separate `2.1forge` history. Apply and validate shared fixes independently in both branches.
...
```bash
git switch 2.1fabric
./gradlew --version
./gradlew --no-daemon --console=plain clean check build
```

The primary JAR is `build/libs/fabricwebmap-0.2.1.jar`. Put it and Fabric API in the server `mods/` directory. Configuration is created at `config/fabricwebmap-common.properties`.
