...
```bash
git switch 2.2fabric
./gradlew --version
./gradlew --no-daemon --console=plain clean check build
```

The primary development JAR is `build/libs/fabricwebmap-0.2.2.jar`. Put it and Fabric API in the server `mods/` directory. Configuration is created at `config/fabricwebmap-common.properties`.
